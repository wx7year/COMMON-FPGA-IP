...
from ref_scl_decode import f, g, saturate, pm_inc
...
print(f"Frozen: {[i for i in range(N) if frozen_mask[i]]}")
print(f"Info: {[i for i in range(N) if not frozen_mask[i]]}")
...
print(f"\nu_hat: {u_hat}")
err = sum(1 for i in range(K) if u_hat[q_n[N-K+i]] != u[q_n[N-K+i]])
print(f"errors: {err}/{K}")
