...
def sc_decode_recursive(llr, N, frozen_mask):
    """递归 SC 译码"""
    u_hat = [0]*N
    ...
    def decode(alpha, start, length):
        if length == 1:
            ...
            if frozen_mask[start]:
                ...
                u_hat[start] = 0 if alpha[0] >= 0 else 1
            return
        
        half = length // 2
        # 左子树 LLR
        left_alpha = [saturate(f(alpha[i], alpha[i+half])) for i in range(half)]
        decode(left_alpha, start, half)
        
        # 右子树 LLR（使用左子树的判决）
        right_alpha = [saturate(g(alpha[i], alpha[i+half], u_hat[start+i])) for i in range(half)]
        decode(right_alpha, start+half, half)
