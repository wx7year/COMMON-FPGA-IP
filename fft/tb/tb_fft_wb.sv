...
module tb_fft_wb;
    ...
    initial begin
        ...
        for (cycle = 0; cycle < 60; cycle = cycle+1) begin
            @(posedge clk);
            // 监控写回
            if (dut.u_core.pipe_v[6]) begin
                if (dut.u_core.wb_bank_pipe[6] == 0)
                    $display("WB bank0[%0d]=%d [%0d]=%d",
                             dut.u_core.addr_a_pipe[6], dut.u_core.y1_re_t,
                             dut.u_core.addr_b_pipe[6], dut.u_core.y2_re_t);
                else
                    $display("WB bank1[%0d]=%d [%0d]=%d",
                             dut.u_core.addr_a_pipe[6], dut.u_core.y1_re_t,
                             dut.u_core.addr_b_pipe[6], dut.u_core.y2_re_t);
            end
